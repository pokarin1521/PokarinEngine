...
namespace PokarinEngine
{
	...
	void ImGuiHelper::Initialize(WindowID windowID)
	{
		// GLFWに対応するための初期化
		// GLFWコールバックは自動作成にする
	...
	void ImGuiHelper::Update(ImGuiContext* context)
	{
		// コンテキストを指定
	...
	/// </summary>
	/// <param name="[in] context"> 使用するImGuiコンテキスト </param>
	void ImGuiHelper::Render(ImGuiContext* context)
	{
		// コンテキストの指定
		ImGui::SetCurrentContext(context);
	...
	/// </summary>
	/// <param name="[in] context"> 使用するImGuiコンテキスト </param>
	void ImGuiHelper::Finalize(ImGuiContext* context)
	{
		// コンテキストを指定
		ImGui::SetCurrentContext(context);
	...
	void ImGuiHelper::PushColor(ImGuiCol style, const Color& styleColor)
	{
		// 色を設定
	...
	void ImGuiHelper::SetFont(ImGuiIO& io)
	{
		// ----------------------------------------------
		// デフォルトのフォントを削除する
		// ----------------------------------------------
