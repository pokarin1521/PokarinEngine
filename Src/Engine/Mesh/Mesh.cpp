			...
			const std::string& folderName, const char* fileName)
		{
			...
			while (!file.eof())
			{
				...
				if (sscanf(line.data(), " map_Kd %999s", &textureName) == 1)
				{
					...
					{
						// テクスチャを取得する
						pMaterial->texBaseColor = TextureGetter::Get(fullpath.c_str());
					}
					// 存在しない
					else
					{
				...
				if (sscanf(line.data(), " map_Ke %999s", &textureName) == 1)
				{
					...
					{
						// エミッションテクスチャを読み込む
						pMaterial->texEmission = TextureGetter::Get(filename.c_str());
					}
					// ファイルが存在しない
					else
					{
			...
			if (!materials[0]->texBaseColor)
			{
				...
				{
					// テクスチャを読み込む
					materials[0]->texBaseColor = TextureGetter::Get(path);
				}
				// 存在しない
				else
				{
...
#pragma region Mesh
	...
	namespace Mesh
	{
		...
		void CreateMeshBuffer()
		{
			...
			// メッシュバッファを作成する
			meshBuffer = MeshBuffer::Create(meshBufferSize);
		}
