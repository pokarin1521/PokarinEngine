...
namespace PokarinEngine
{
	...
	TexturePtr TextureGetter::Get(const std::string& fileName)
	{
		...
		// テクスチャ
		// ファイルからテクスチャを読み込む
		TexturePtr tex = LoadTexture(fileName);
	...
	TexturePtr TextureGetter::LoadTexture(const std::string& fileName)
	{
		// ---------------------
		// ファイルを開く
		// ---------------------
		...
		// テクスチャの幅
		int textureWidth = buffer[12] + buffer[13] * 256;

		// テクスチャの高さ
		int textureHeight = buffer[14] + buffer[15] * 256;
		...
		if (imageType & 0b1000)
		{
			...
			// 展開後のバイト数
			const int imageBytes = textureWidth * textureHeight * pixelBytes;
		...
		if (topToBottom)
		{
			...
			// 一行のバイト数
			const int lineByteSize = textureWidth * pixelDepth / 8;
			...
			// 一番下の行の位置を求める
			uint8_t* bottom = top + lineByteSize * (textureHeight - 1); // 下の行の位置
		...
		// 1行のバイト数
		const int lineByteSize = textureWidth * pixelBytes;
		...
		glTextureStorage2D(textureID, 1, format->gpuFormat, textureWidth, textureHeight);
		...
		glTextureSubImage2D(textureID, 0, 0, 0, textureWidth, textureHeight,
		...
		{
			glTextureParameteri(textureID, GL_TEXTURE_SWIZZLE_R, GL_RED);
